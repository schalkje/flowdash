...
test.describe('demo-nav.js', () => {
    ...
    test('renders on pages whose .demo-header is built at runtime via mountDemoChrome', async ({ page }) => {
    ...
    test('inside the index iframe, Next click updates the sidebar active link', async ({ page }) => {
        ...
        }, { timeout: 8000 }).toBe(nextPath);
        // And the iframe's URL should now point at nextPath.
        await expect.poll(async () => {
            return await page.evaluate(() => {
                const v = document.getElementById('viewer');
                try { return v.contentWindow.location.pathname; } catch { return null; }
            });
        }, { timeout: 8000 }).toBe('/' + nextPath);
    });
});
