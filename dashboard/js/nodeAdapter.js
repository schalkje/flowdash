...
export default class AdapterNode extends BaseContainerNode {
  ...
  static initializeNodeDataStatic(nodeData) {
    ...
    if (nodeData.layout.arrangement === undefined || nodeData.layout.arrangement === null) {
      ...
      if (typeof arr !== 'number' || !Number.isInteger(arr) || !validArrangements.includes(arr)) {
        console.error(
          `AdapterNode: invalid layout.arrangement value: "${arr}". Expected number in [1..5]. Defaulting to 1.`,
        );
        nodeData.layout.arrangement = 1;
      }
    }
    ...
    if (nodeData.children.length > 0) {
      requiredChildren.forEach((role) => {
        const exists = nodeData.children.some(
          (c) => (c.role || '').toLowerCase() === role || (c.category || '').toLowerCase() === role,
        );
        if (!exists) {
          const childData = {
  ...
  initializeChildNode(role, labels) {
    ...
    if (!node) {
      node = this.childNodes.find(
        (child) =>
          child.data.role === role ||
          labels.some(
            (label) => child.data.label && child.data.label.toLowerCase().includes(label),
          ),
      );
    }

    if (!node) {
      let childData = this.data.children.find(
        (child) => child.category === role || child.role === role,
      );
  ...
  layoutAlgorithm1_full_archive(childNodes, coordinateSystem) {
    ...
    if (stagingNode && archiveNode && transformNode) {
      ...
      const requiredArchiveWidth =
        transformNode.data.width + horizontalSpacing + (1 / 3) * stagingNode.data.width;
      ...
      const totalContentHeight =
        stagingNode.data.height + verticalSpacing + archiveNode.data.height;
      ...
      const transformCenterX =
        stagingCenterX +
        stagingNode.data.width / 2 +
        horizontalSpacing +
        transformNode.data.width / 2;
    ...
    if (marginZone) {
      ...
      const newSize = {
        width: Math.max(
          ...
          headerMinWidth,
          contentWidth + adjustedMargins.left + adjustedMargins.right,
        ),
  ...
  layoutAlgorithm3_full_staging(childNodes, coordinateSystem) {
    ...
    if (stagingNode && transformNode && archiveNode) {
      ...
      const stagingCenterX = -totalWidth / 2 + stagingNode.data.width / 2;
      const rightColumnCenterX =
        stagingCenterX + stagingNode.data.width / 2 + horizontalSpacing + rightColumnWidth / 2;
  ...
  layoutAlgorithm4_line(childNodes, coordinateSystem) {
    ...
    if (stagingNode && otherNode) {
      ...
      const totalWidth =
        stagingNode.data.width + this.nodeSpacing.horizontal + otherNode.data.width;
