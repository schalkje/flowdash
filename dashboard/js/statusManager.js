...
export class StatusManager {
  static calculateContainerStatus(childNodes, settings) {
    ...
    // Use direct children's statuses — not a recursive leaf walk. If callers
    // process bottom-up (initializeChildrenStatusses) or propagate one ancestor
    // at a time (cascadeStatusChange), each child's status already reflects its
    // subtree, so direct-child aggregation produces the same result as a full
    // leaf walk. The previous recursive implementation was O(N²) across an
    // initial bottom-up pass; this is O(N).
    const statuses = childNodes.map((n) => n.status);
  ...
  static determineAggregateStatus(statuses) {
    ...
    const onlySkippedAndUpdated = uniqueStatuses.every(
      (s) => s === NodeStatus.SKIPPED || s === NodeStatus.UPDATED,
    );
  ...
  static shouldCollapseOnStatus(status, settings) {
    ...
    return [NodeStatus.READY, NodeStatus.DISABLED, NodeStatus.UPDATED, NodeStatus.SKIPPED].includes(
      status,
    );
  }
  ...
  static isProcessStatus(status) {
    return [NodeStatus.READY, NodeStatus.UPDATING, NodeStatus.UPDATED, NodeStatus.SKIPPED].includes(
      status,
    );
  }
}
