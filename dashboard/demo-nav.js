...
(function () {
    ...
    function navigateTo(path) {
        ...
        if (inIframe()) {
            try {
                ...
                return;
            } catch (e) { /* fall through to top-level navigation */ }
        }
    ...
    function parsePaths(html) {
        ...
        var anchors = doc.querySelectorAll('a[data-path]');
        var paths = [];
    ...
    function build(paths) {
        ...
        var idx = paths.indexOf(here);
        var prev = idx > 0 ? paths[idx - 1] : null;
        var next = (idx >= 0 && idx < paths.length - 1) ? paths[idx + 1] : null;
        ...
        document.addEventListener('keydown', function (e) {
            ...
            if (e.key === 'ArrowLeft' && prev) { e.preventDefault(); navigateTo(prev); }
            else if (e.key === 'ArrowRight' && next) { e.preventDefault(); navigateTo(next); }
        });
    }
    ...
    function whenHeaderReady(cb) {
        var existing = document.querySelector('.demo-header');
        if (existing) { cb(existing); return; }
        if (!document.body) { document.addEventListener('DOMContentLoaded', function () { whenHeaderReady(cb); }); return; }
        var done = false;
        var timer = null;
        var obs = new MutationObserver(function () {
            var h = document.querySelector('.demo-header');
            if (h && !done) { done = true; obs.disconnect(); if (timer) clearTimeout(timer); cb(h); }
        });
        obs.observe(document.body, { childList: true, subtree: true });
        // Give up after a few seconds — the page just doesn't have a header.
        timer = setTimeout(function () { if (!done) { done = true; obs.disconnect(); } }, 5000);
    }

    function start() {
        injectStyle();
        var paths = null, header = null;
        function tryBuild() { if (paths && header) build(paths); }
        fetch('/index.html', { cache: 'no-store' })
            .then(function (r) { return r.ok ? r.text() : Promise.reject(new Error('HTTP ' + r.status)); })
            .then(function (html) { paths = parsePaths(html); tryBuild(); })
            .catch(function (err) { console.warn('[demo-nav] could not load index.html:', err); });
        whenHeaderReady(function (h) { header = h; tryBuild(); });
    }
