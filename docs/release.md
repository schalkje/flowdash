...
| `flowdash-harness` | [`/package.json`](../package.json)                     | Test runner, demo server orchestrator, and dev tooling. **Not published, not consumed by anyone.**                                                                         | Stays pinned, marked `"private": true`. Don't bump it — a `preversion` script in root `package.json` aborts `npm version` if run from the repo root. |
